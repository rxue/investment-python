...
@cache
def fetch_fx_rate_from_euro(currency: str, target_date: date) -> tuple[date, Decimal]:
    """Fetch the EUR-to-``currency`` exchange rate for ``date`` from the
    ECB (European Central Bank) - i.e. how many ``currency`` units 1 EUR
    (the base currency) is worth.

    Returns ``1.0`` for ``date`` unchanged when ``currency`` is already ``'EUR'``.
    ...
    :param currency: ISO 4217 currency code to convert to (e.g. ``'USD'``).
    ...
    row = next(reader)
    return target_date.fromisoformat(row["TIME_PERIOD"]), Decimal(row["OBS_VALUE"])

@cache
def fetch_fx_rate_series_from_euro(
    ...
    unchanged when ``currency`` is already ``'EUR'``.
    ...
    url = f"https://data-api.ecb.europa.eu/service/data/EXR/D.{currency}.EUR.SP00.A"
    logger.info(f"Fetch FX rate series for {currency} from {start_date} to {end_date}")
    response = requests.get(url, params={
