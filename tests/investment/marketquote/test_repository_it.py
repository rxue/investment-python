...
def test_fetch_historical_prices_converts_to_eur_by_default_for_a_usd_stock():
    """PFE is quoted in USD. With no ``currency`` argument given, the
    ``currency=EUR`` default should trigger a day-by-day FX conversion so
    the returned series comes back in EUR rather than PFE's native USD.
    """
    end = date(2025, 12, 31)
